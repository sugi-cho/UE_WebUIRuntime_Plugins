...
namespace
{
	...
	const double RenderTargetStreamIntervalSeconds = 0.1;
	...
	public:
		...
		void SetSubscription(const FString& InWebUIId, const TArray<FString>& InImageKeys)
		{
			SubscribedWebUIId = InWebUIId;
			SubscribedImageKeys.Reset();
			for (const FString& ImageKey : InImageKeys)
			{
				if (!ImageKey.IsEmpty())
				{
					SubscribedImageKeys.Add(ImageKey);
				}
			}
			StreamLastSentTimes.Reset();
		}

		void ClearSubscription()
		{
			SubscribedWebUIId.Reset();
			SubscribedImageKeys.Reset();
			StreamLastSentTimes.Reset();
		}

		bool WantsFrame(const FString& WebUIId, const FString& ImageKey) const
		{
		...
		bool CanSendFrame(const FString& WebUIId, const FString& ImageKey, double Now, double StreamIntervalSeconds) const
		{
			...
			const double* LastSentTime = StreamLastSentTimes.Find(ImageKey);
			return !LastSentTime || (Now - *LastSentTime) >= StreamIntervalSeconds;
		}

		void MarkFrameSent(const FString& ImageKey, double Now)
		{
			StreamLastSentTimes.Add(ImageKey, Now);
		}
	...
	private:
		...
		FString SubscribedWebUIId;
		TSet<FString> SubscribedImageKeys;
		TMap<FString, double> StreamLastSentTimes;
...
class FWebUIRenderTargetWebSocketServer : public FTickableGameObject
{
	public:
		...
		void SendToSubscribedConnections(const FString& WebUIId, const FString& ImageKey, const FString& Message, double Now, double StreamIntervalSeconds)
		{
			for (const TPair<uint16, TUniquePtr<FWebUIWebSocketConnection>>& Pair : Connections)
			{
				if (Pair.Value.IsValid() && Pair.Value->CanSendFrame(WebUIId, ImageKey, Now, StreamIntervalSeconds))
				{
					if (Pair.Value->Send(Message))
					{
						Pair.Value->MarkFrameSent(ImageKey, Now);
					}
				}
			}
		}

		bool HasReadyConnectionForFrame(const FString& WebUIId, const FString& ImageKey, double Now, double StreamIntervalSeconds) const
		{
			for (const TPair<uint16, TUniquePtr<FWebUIWebSocketConnection>>& Pair : Connections)
			{
				if (Pair.Value.IsValid() && Pair.Value->CanSendFrame(WebUIId, ImageKey, Now, StreamIntervalSeconds))
	...
	protected:
		...
		void OnPacketReceived(void* Data, int32 Size, uint16 ConnectionId)
		{
			...
			TUniquePtr<FWebUIWebSocketConnection>* ConnectionPtr = Connections.Find(ConnectionId);
			if (!ConnectionPtr || !ConnectionPtr->IsValid())
			{
				return;
			}
			FWebUIWebSocketConnection* Connection = ConnectionPtr->Get();

			const FUTF8ToTCHAR Converted(reinterpret_cast<const ANSICHAR*>(Data), Size);
			...
			if (MessageType.Equals(TEXT("subscribe"), ESearchCase::IgnoreCase))
			{
				FString WebUIId;
				if (!Message->TryGetStringField(TEXT("webUIId"), WebUIId))
				{
					return;
				}

				TArray<FString> ImageKeys;
				const TArray<TSharedPtr<FJsonValue>>* ImageKeyValues = nullptr;
				if (Message->TryGetArrayField(TEXT("imageKeys"), ImageKeyValues))
				{
					for (const TSharedPtr<FJsonValue>& Value : *ImageKeyValues)
					{
						FString ImageKey;
						if (Value.IsValid() && Value->TryGetString(ImageKey) && !ImageKey.IsEmpty())
						{
				...
				Connection->SetSubscription(WebUIId, ImageKeys);
			}
			else if (MessageType.Equals(TEXT("unsubscribeAll"), ESearchCase::IgnoreCase))
			{
				Connection->ClearSubscription();
			}
			else if (MessageType.Equals(TEXT("snapshotRequest"), ESearchCase::IgnoreCase))
			{
...
:root{
 ...
 html,body{margin:0;height:100%;background:transparent;overflow:hidden;overflow-x:hidden}
 body{height:100%;font-family:system-ui,sans-serif;color:var(--text);overscroll-behavior-x:none}
body.theme-light{--page-bg:rgba(248,250,252,0.98);--shell-bg:#f8fafc;--panel-bg:#ffffff;--panel-bg-embed:rgba(255,255,255,0.74);--border:#d8e0ea;--border-subtle:rgba(16,18,22,.10);--border-strong:#7aa3d8;--button-bg:#e6edf6;--button-bg-pressed:#bfd6ef;--text:#101216;--muted:#4b5563}
body.theme-dark{--page-bg:rgba(16,18,22,0.98);--shell-bg:#101216;--panel-bg:#171b22;--panel-bg-embed:rgba(23,27,34,0.72);--border:#2d3440;--border-subtle:rgba(255,255,255,.10);--border-strong:#5b7898;--button-bg:#233247;--button-bg-pressed:#4b6f95;--text:#e9edf2;--muted:#cfd6df}
 body.embed{overflow:hidden}
 body.embed{background:transparent}
 body.compact{--page-padding:12px;--content-gap:10px}
  :root{--webui-viewport-height:100vh}
  .page-shell{height:var(--webui-viewport-height);box-sizing:border-box;padding:var(--page-padding);background:linear-gradient(180deg,rgba(255,255,255,.03),transparent 26%),var(--shell-bg);display:flex;flex-direction:column;min-height:0;min-width:0}
  body.embed .page-shell{height:var(--webui-viewport-height);padding:0;background:transparent}
 .page-header{display:flex;justify-content:space-between;align-items:center;gap:12px;margin:0 0 var(--content-gap);min-width:0}
 .page-title{margin:0}
 ...
 .app-shell{display:flex;flex-direction:column;flex:1;min-height:0;min-width:0}
 ...
 .panel-scroll{flex:1;min-height:0;display:flex;min-width:0}
 .panel-scroll > div{flex:1;min-height:0;display:flex;min-width:0}
 .panel{display:none;flex:1;min-height:0;min-width:0;flex-direction:column;border:1px solid var(--border);border-top:none;border-radius:0 var(--panel-radius) var(--panel-radius) var(--panel-radius);background:var(--panel-bg);overflow:hidden}
...
.panel-body{flex:1;min-height:0;min-width:0;overflow:auto;overflow-x:hidden;padding:0 16px 16px;scrollbar-width:thin;scrollbar-color:transparent transparent;overscroll-behavior:contain;overscroll-behavior-x:none}
...
.host-meta{opacity:.78;margin-top:4px;white-space:pre-wrap;overflow-wrap:anywhere}
.host-header-info{display:flex;flex-direction:column;gap:8px;min-width:0}
.host-inline-editor{display:flex;flex-direction:column;gap:8px;min-width:0}
.host-inline-editor .property-row:first-child{margin-top:0}
.host-inline-editor .button-list{margin:0}
.component{border-top:1px solid var(--border);padding-top:12px;margin-top:12px;min-width:0}
.property-row{display:grid;grid-template-columns:180px minmax(0,1fr);column-gap:12px;row-gap:6px;align-items:center;margin:10px 0;padding:10px 0;border-bottom:1px solid var(--border-subtle);min-width:0}
.property-name{padding-top:2px;min-width:0;overflow-wrap:anywhere;word-break:break-word}
.property-control{display:flex;flex-wrap:wrap;gap:8px;align-items:center;justify-content:flex-start;min-width:0;width:100%;max-width:100%}
input,button,select{font:inherit;padding:8px;border-radius:6px;border:1px solid var(--border);background:#0d1015;color:var(--text);box-sizing:border-box;min-width:0;max-width:100%}
...
input[type="range"]{flex:1 1 240px;min-width:180px;padding:8px 0;max-width:100%}
.numeric-field{width:120px}
.option-picker{position:relative;flex:1 1 260px;min-width:180px;max-width:100%}
.option-picker-button{width:100%;display:flex;justify-content:space-between;gap:8px;text-align:left;white-space:normal;overflow-wrap:anywhere}
...
.vector-group,.rotator-group,.color-group,.linear-color-group{display:grid;width:100%;align-items:center;min-width:0;max-width:100%}
...
@media (max-width: 768px), (max-height: 720px){
 ...
 body{font-size:14px;line-height:1.32;background:var(--page-bg)}
 body.embed{background:transparent}
 .page-shell{padding-top:max(6px, env(safe-area-inset-top));padding-right:max(6px, env(safe-area-inset-right));padding-bottom:max(6px, env(safe-area-inset-bottom));padding-left:max(6px, env(safe-area-inset-left));background:linear-gradient(180deg,rgba(255,255,255,.04),transparent 30%),var(--shell-bg)}
 .page-header{gap:8px;margin-bottom:8px;padding:8px 10px;border:1px solid var(--border);border-radius:14px;background:rgba(255,255,255,.04);backdrop-filter:saturate(120%) blur(8px)}
 .page-title{font-size:13px;letter-spacing:.04em;text-transform:uppercase}
 .external-link{display:none}
 .tabs{padding-left:4px;gap:4px;margin-top:8px}
 .tab{padding:8px 10px;border-radius:10px 10px 0 0}
.panel-header{padding:10px 10px 0}
.panel-body{padding:0 10px 10px}
.host-meta{margin-top:2px}
.host-header-info{gap:6px}
.host-inline-editor{gap:6px}
.component{padding-top:10px;margin-top:10px}
 .property-row{grid-template-columns:minmax(84px,34vw) minmax(0,1fr);column-gap:8px;row-gap:4px;margin:8px 0;padding:8px 0;border-bottom:1px solid var(--border-subtle)}
 .property-name{padding-top:0;font-size:13px}
 .property-control{gap:6px}
 input,button,select{padding:7px 8px;font-size:14px}
 input[type="range"]{min-width:0;flex:1 1 100%}
 .numeric-field{width:100%}
 .option-picker{flex:1 1 100%;min-width:0}
 .option-picker-button{white-space:normal}
 .button-list{gap:6px;margin:6px 0}
 button.webui-button{min-width:0;flex:1 1 120px}
 .preview-layout{flex-direction:row;gap:8px}
...
function syncViewportHeight(){
 ...
 if(viewportHeight>0){
  document.documentElement.style.setProperty('--webui-viewport-height',`${viewportHeight}px`);
 }
}
...
function syncScrollContainerToBottom(scrollRoot){
 if(!scrollRoot){
  return;
 }
 const maxScrollTop=Math.max(0,scrollRoot.scrollHeight-scrollRoot.clientHeight);
 if(scrollRoot.scrollTop>=maxScrollTop-2){
  scrollRoot.scrollTop=maxScrollTop;
 }
}
function refreshScrollLayout(scrollRoot){
 if(!scrollRoot){
  return;
 }
 if(layoutRefreshTimer){
  cancelAnimationFrame(layoutRefreshTimer);
 }
 layoutRefreshTimer=requestAnimationFrame(()=>{
  layoutRefreshTimer=requestAnimationFrame(()=>{
   layoutRefreshTimer=null;
   syncScrollContainerToBottom(scrollRoot);
  });
 });
}
...
function buildImageRegistryKey(webUIId,componentId,slot){
 return `${String(webUIId || '')}::${String(componentId || '')}::${String(slot || '')}`;
}
function collectSubscribedImageKeys(webUIId){
 const prefix=`${String(webUIId || '')}::`;
 const keys=[];
 for(const key of imageElementRegistry.keys()){
...
function sendRenderTargetMessage(message){
 if(!renderTargetSocket || renderTargetSocket.readyState!==WebSocket.OPEN){
  return false;
 }
 try{
  renderTargetSocket.send(JSON.stringify(message));
  return true;
 }catch(error){
  console.warn('Failed to send render target control message', error);
  return false;
 }
}
)HTML");
...
function syncRenderTargetSubscription(){
 ...
 if(!currentWebUIId){
  sendRenderTargetMessage({type:'unsubscribeAll'});
  return;
 }
 sendRenderTargetMessage({type:'subscribe',webUIId:currentWebUIId,imageKeys:collectSubscribedImageKeys(currentWebUIId)});
 sendRenderTargetMessage({type:'snapshotRequest'});
}
function applyRenderTargetFrame(message){
 const key=buildImageRegistryKey(message.webUIId,message.componentId,message.slot);
...
function renderImageFrame(webUIId,image,variant){
 ...
 const handleImageLayoutChange=()=>{
  refreshScrollLayout(frame.closest('.panel-body') || scrollContainer);
 };
 img.onload=()=>{
...
function renderStaticSection(title,contentBuilder){
 const section=document.createElement('section');
 section.className='component';
 const body=document.createElement('div');
 body.className='component-body';
...
function renderInlineActorEditor(host){
 const editor=document.createElement('div');
 editor.className='host-inline-editor';
 if((host.actorProperties||[]).length){
  editor.append(renderProperties(host,'actor','',host.actorProperties));
 }
 if((host.actorButtons||[]).length){
  editor.append(renderButtonRow(host,'actor','',host.actorButtons));
 }
 if((host.hostButtons||[]).length){
  editor.append(renderButtonRow(host,'host','',host.hostButtons));
 }
 return editor;
}
	)HTML");
...
async function load(){
 ...
 for(const host of hosts){
  ...
  tab.onclick=()=>setActive(host.webUIId);
   ...
   headerInfo.className='host-header-info';
   const headerTitle=document.createElement('h2');
   headerTitle.textContent=host.displayName || host.webUIId;
   const headerMeta=document.createElement('div');
   headerMeta.className='host-meta';
   headerMeta.textContent=host.description || host.actorName;
   headerInfo.append(headerTitle,headerMeta);
   if((host.actorProperties||[]).length || (host.actorButtons||[]).length || (host.hostButtons||[]).length){
    headerInfo.append(renderInlineActorEditor(host));
   }
...
bool UWebUIRuntimeSubsystem::TickWebSocketStreaming(float DeltaTime)
{
	...
	for (UWebUIHostComponent* Host : Hosts)
	{
		...
		for (UActorComponent* Component : Components)
		{
			...
			const FString SlotName = StaticEnum<EWebUIImageSlot>()->GetNameStringByValue(static_cast<int64>(ImageComponent->WebUIImageSlot));
			const FString StreamKey = BuildRenderTargetStreamKey(WebUIId, ImageComponent->GetName(), SlotName);
			if (!WebSocketServer->HasReadyConnectionForFrame(WebUIId, StreamKey, Now, StreamIntervalSeconds))
			{
				continue;
			}
			...
			Message->SetStringField(TEXT("slot"), SlotName);
			...
			WebSocketServer->SendToSubscribedConnections(WebUIId, StreamKey, JsonString, Now, StreamIntervalSeconds);
		}
	}
